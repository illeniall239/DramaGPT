...
class KBChartGenerator:
        ...
        Returns:
            {
                ...
                'description': 'Descriptive text about the visualization'
            }
        """
        try:
            ...
            if visualization:
                ...
                description = self._generate_chart_description(query, df, visualization['type'])
    ...
    def _generate_chart_description(self, query: str, df: pd.DataFrame, chart_type: str) -> str:
        """
        Generate descriptive text explaining the visualization.
        ...
        try:
            if not self.llm:
                return f"Visualization created with {df.shape[0]} data points."

            prompt = f"""Generate a brief, informative description (1-2 sentences) for this data visualization.
...
Generate ONLY the description text, no additional formatting or explanations."""
            ...
            response = response.replace('**', '').replace('*', '').strip('"').strip("'")
            return response

        except Exception as e:
            logger.error(f"Error generating description: {e}")
            return f"Visualization showing {df.shape[0]} data points from your query."
    ...
    def _parse_sql_results_to_dataframe(self, sql_results: str) -> pd.DataFrame:
        ...
        - Python list/tuple format: [('value1', 123), ('value2', 456)]
        ...
        """
        try:
            # Try Python list/tuple format: [('Drama1', 5083.572), ('Drama2', 4459.422)]
            if sql_results.strip().startswith('[') and '(' in sql_results:
                try:
                    import ast
                    data = ast.literal_eval(sql_results.strip())
                    if isinstance(data, list) and len(data) > 0:
                        # Convert list of tuples to DataFrame
                        # Try to infer column names from context or use generic names
