...
/**
 * Generate a meaningful chat title using AI based on the first user message
 */
...
/**
 * Load all knowledge bases (single-user mode)
 */
export async function loadKnowledgeBases(): Promise<any[]> {
    try {
        const response = await fetch(`${API_BASE_URL}/api/kb/list`);
        ...
        const data = await response.json();
        console.log('📚 Loaded knowledge bases:', data.knowledge_bases?.length || 0);
...
/**
 * Delete a chat from a knowledge base
 */
export async function deleteKBChat(chatId: string): Promise<void> {
    try {
        const { supabase } = await import('@/utils/supabase');
        ...
        if (error) {
            console.error('Error deleting KB chat:', error);
            throw error;
        }

        console.log('✅ KB chat deleted:', chatId);
    } catch (error) {
        console.error('Error in deleteKBChat:', error);
        throw error;
    }
}
 ...
 * Note: Documents are created by the Python backend, not Supabase.
 *       Fetch directly from the API to stay in sync with uploads.
 */
export async function loadKBDocuments(kbId: string): Promise<any[]> {
    try {
        console.log('📄 Loading documents for KB:', kbId);

        const response = await fetch(`${API_BASE_URL}/api/kb/${kbId}/documents`, {
            method: 'GET',
        });

        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            throw new Error(errorData.detail || `Failed to load documents: ${response.statusText}`);
        }

        const data = await response.json();
        console.log('✅ Loaded KB documents:', data.documents?.length || 0);
...
export async function deleteKBDocument(kbId: string, documentId: string): Promise<void> {
    try {
        console.log('🗑️ Deleting document:', { kbId, documentId });

        const response = await fetch(`${API_BASE_URL}/api/kb/${kbId}/documents/${documentId}`, {
            method: 'DELETE',
        });
 ...
 */
export async function saveKBChatMessages(chatId: string, messages: any[]): Promise<void> {
    try {
        const { supabase } = await import('@/utils/supabase');

        console.log('💾 Saving KB chat messages:', chatId, messages.length);
        ...
        if (error) {
            console.error('Error saving KB chat messages:', error);
            throw error;
        }

        console.log('✅ KB chat messages saved successfully');
    } catch (error) {
        console.error('Error in saveKBChatMessages:', error);
        throw error;
    }
}
 ...
 */
export async function loadKBChatMessages(chatId: string): Promise<any[]> {
    try {
        const { supabase } = await import('@/utils/supabase');

        console.log('📥 Loading KB chat messages:', chatId);
        ...
        if (error) {
            console.error('Error loading KB chat messages:', error);
            return [];
        }

        const messages = data?.messages || [];
        console.log('✅ KB chat messages loaded:', messages.length);
        return messages;
    } catch (error) {
        console.error('Error in loadKBChatMessages:', error);
        return [];
    }
}